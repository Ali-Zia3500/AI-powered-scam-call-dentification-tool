...
class ModelService:
    ...
    def __init__(self):
        ...
        # Get the project root directory (one level up from flask_sentinel)
    ...
    def analyze_text(self, text):
        ...
        if scam_score > 0.5 or (linguistic_features.get('authority', 0) > 0 and linguistic_features.get('sensitivity', 0) > 0):
            risk_level = "critical"
        elif scam_score > 0.3 or linguistic_features.get('sensitivity', 0) > 0:
            risk_level = "warning"
        else:
            risk_level = "safe"
